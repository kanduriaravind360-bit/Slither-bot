...
camera.start(region=(
    monitor_box["left"],
    monitor_box["top"],
    monitor_box["left"] + monitor_box["width"],
    monitor_box["top"] + monitor_box["height"]
))
cv2.namedWindow("Slither")
while True:
    ...
    with frame_lock:
        latest_frame = frame
    dfood_data = dfood(frame, hsv)
    wall_data = wall(frame, hsv)
    threat_data = threat(frame, hsv, gray)
    foods = food(frame, hsv)
    threats = threat_data
    ...
    display = cv2.resize(frame, (1200, 675), interpolation=cv2.INTER_AREA)
    cv2.imshow("Slither", display)
